...
if (['ьгеу', 'mute', 'мут'].includes(command) && message.member.roles.some(r=>[moder, owner].includes(r.id))) {
    ...
    if(['piar'].includes(command) && message.member.roles.some(r=>[owner].includes(r.id))) {
        bot.fetchUser('242975403512168449', '285372240198107136', '428036906723573760').then (user => user.send('Автопиар тест'));
        bot.fetchUser('421030089732653057').then (user => user.send('Кто сказал что невозможно?'));
